...
def page_brain_tumor_detector_body():
    ...
    st.info(
        ...
        f"differentiate between a normal brain and one with Glioma, "
        f"Meningioma, or Pituitary tumors."
    )

    st.write(
        f"* You can download a set of classified brain tumor MRI images for"
        f"live prediction. "
        f"You can download the images from [this Kaggle dataset]"
        f"(https://www.kaggle.com/datasets/masoudnickparvar/"
        f"brain-tumor-mri-dataset)."
    )

    st.write("---")

    images_buffer = st.file_uploader(
            'Upload Brain MRI Image. You may select more than one.',
    ...
    if images_buffer is not None:
        for image in images_buffer:
            ...
            st.image(img_pil, caption=f"Image Size: {img_array.shape[1]}px "
                     f"width x {img_array.shape[0]}px height")
